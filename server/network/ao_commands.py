...
def net_cmd_yaml_char(client: ClientManager.Client, pargs: Dict[str, Any]):
    if not client.is_staff():
        client.send_ooc('You do not have permission to set the character list.')
        return

    yaml_hash = pargs['yaml_data']

    save_dir = os.path.join('config', 'char_lists')
    os.makedirs(save_dir, exist_ok=True)
    ...
    try:
        ...
        logger.log_server(f"Character YAML saved to {yaml_file_path}", client)
        client.hub.character_manager.command_list_load(client, yaml_file_name)
